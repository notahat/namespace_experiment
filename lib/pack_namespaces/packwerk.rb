# Teaches packwerk about the pack namespaces described in layout.rb and
# applied to Zeitwerk in railtie.rb. Loaded through the `require` setting in
# packwerk.yml rather than with the rest of the library, since packwerk isn't
# always present.
#
...
#
# - PackwerkLoadPaths replaces each pack root with one load path per
#   namespaced app directory, each mapped to its pack's namespace, so packwerk
#   sees Catalog::Book.
# - PackwerkFileMap adds each namespaced pack's ns.rb to the resolver's file
#   map as the definition of the namespace module itself, so a reference to
#   bare Catalog is attributed to the pack too.
#
...
module PackNamespaces
  ...
  module PackwerkLoadPaths
    ...
    # Returns a hash from each namespaced app directory, relative to the
    # project root, to the namespace module its files define constants in.
    def pack_load_paths
  ...
  module PackwerkFileMap
    ...
    def pack_namespace_files
      PackNamespaces.namespaced_packs.filter_map do |pack|
        namespace_file = pack.path.join(NAMESPACE_FILE)
...
Packwerk::Configuration.prepend(PackNamespaces::PackwerkLoadPaths)
ConstantResolver.prepend(PackNamespaces::PackwerkFileMap)
